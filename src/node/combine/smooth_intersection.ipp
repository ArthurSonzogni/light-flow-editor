...
auto model_smooth_intersection = smkflow::model::Node{
    Node::SmoothIntersection,
    "Smooth Intersection",
    model_type_fusion,
    {
        {"a", type_sdf},
...
std::string BuildSmoothIntersection(smkflow::Node* node,
                               const std::string& in,
                               const std::string& out,
                               Context* context) {
  context->RegisterFunction(R"(
    Value SmoothIntersection(float k, Value a, Value b) {
      Value value;
